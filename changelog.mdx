...
- **Message Tag Filtering**: Messages can now be filtered by tags in both the [API](/api-reference/messages/list-messages) and the [dashboard](https://dash.ahasend.com/account/-/transactional/messages), making it easier to find specific transactional messages. The dashboard includes a multi-select tag dropdown and a `tags=x|y` search bar syntax.
