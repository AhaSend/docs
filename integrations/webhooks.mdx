---
title: "Email Webhooks: Delivery, Bounce and Open Events"
sidebarTitle: "Webhooks"
description: "Receive real-time AhaSend email events (delivered, bounced, deferred, opened, clicked) with webhooks, verify signatures and handle retries."
icon: "webhook"
---


AhaSend webhooks send email events to your application as HTTP requests; this guide shows how to create an endpoint, choose events and test delivery.
...
<a id="available-webhook-events" />

## Which Events Can I Receive?

Choose delivery, bounce, deferral, failure, suppression, open, click and domain-DNS events when configuring your webhook. The [event reference](/api-reference/webhooks#event-types) lists every event, its filter flag and its payload. Inbound routes use the separate [`message.routing` event](/api-reference/routes/route-message).
...
<a id="testing-your-webhooks" />

## How Do I Test a Webhook Locally?

Use [CLI webhook testing](/cli/webhook-testing) to forward events to localhost and trigger sample events. Use [sandbox sends](/send-api/sandbox) to exercise delivery outcomes without real mail.
...
<a id="security-and-verification" />
...
- [Domain setup](/domains), [API authentication](/api-reference/authentication), [Event reference](/api-reference/webhooks).
...
- [Plans and limits](/facts), [Tracking setup](/tracking/open-tracking), [Sandbox behavior](/send-api/sandbox).
- [Suppression durations](/api-reference/webhooks#suppression-duration).
