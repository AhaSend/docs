...
AhaSend makes 12 attempts over about 11 hours, including the first attempt. Each attempt times out after 5 seconds. After more than 100 consecutive failed attempts, the route is disabled and account users are emailed. See [route retry policy](/api-reference/routes/retry-policy) and [security](/api-reference/routes/security).
...
<CardGroup cols={3}>
  ...
  <Card title="Domain Setup" icon="globe" href="/domains">
    Configure domains and [MX records from your domain details](/domains) for routing
  </Card>
