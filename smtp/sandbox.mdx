---
title: "Test SMTP Email with Sandbox Mode"
sidebarTitle: "Sandbox Mode"
description: "Test SMTP email with AhaSend sandbox credentials to simulate delivery, bounces and suppressions without sending messages to real recipients."
icon: "flask"
---
...
<AccordionGroup>
  <Accordion title="Webhook Events Still Fire" icon="webhook">
    - Delivery outcome webhooks are triggered in sandbox mode; opens and clicks need an actual recipient interaction and are not simulated
    - Test delivery confirmations, deferrals, and bounces
    - Use [CLI webhook triggers](/cli/webhook-testing) to test open and click handlers with synthetic events
...
<AccordionGroup>
  ...
  <Accordion title="Simulated Outcomes Not Working" icon="vial">
    **Common issues:**
    - `AhaSend-Sandbox` accepts `true` or `1`; use the result header with sandbox mode. See [special header behavior](/smtp/special-headers).
