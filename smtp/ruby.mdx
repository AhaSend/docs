---
title: "Send Email in Ruby via SMTP (Rails, net/smtp)"
sidebarTitle: "Ruby"
description: "Send email from Ruby through the AhaSend SMTP relay with net/smtp, the Mail gem or Rails ActionMailer: settings, attachments and sandbox tests."
icon: "gem"
---
...
def send_email_with_headers
  ...
  message = <<~MESSAGE_END
    ...
    AhaSend-Metadata-Retention: 30
    AhaSend-Data-Retention: 7
...
def send_email_with_special_headers
  begin
    mail = Mail.new do
      ...
      header['AhaSend-Metadata-Retention'] = '30'
      header['AhaSend-Data-Retention'] = '7'
...
class EmailService
  ...
  def send_email(to:, subject:, template:, data:, headers: {})
    begin
      text_body = generate_text_template(template, data)
      html_body = generate_html_template(template, data)
      mail = Mail.new do
        from     'hello@yourdomain.com'
        to       to
        subject  subject
...
TestMailer.sandbox_test('test@example.com', 'fail').deliver_now
```
