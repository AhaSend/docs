---
title: "Send Email from the Command Line with Swaks"
sidebarTitle: "Command Line (Swaks)"
description: "Send and test email over AhaSend SMTP from the terminal with Swaks: STARTTLS and auth flags, HTML, attachments, custom headers, sandbox tests and shell scripts."
icon: "terminal"
---
...
wsl --install
# In the Ubuntu WSL shell, follow the Ubuntu/Debian instructions above.
```
...
swaks \
  ...
  --add-header "AhaSend-Metadata-Retention: 7" \
  --add-header "AhaSend-Data-Retention: 1" \
...
swaks \
  ...
  --add-header "AhaSend-Metadata-Retention: 30" \
  --add-header "AhaSend-Data-Retention: 7" \
...
<CardGroup cols={2}>
  ...
  <Card title="SMTP Testing Guide" icon="flask" href="/send-api/sandbox">
    Complete guide to testing email delivery
  </Card>
  <Card title="AhaSend SMTP Settings" icon="server" href="/smtp">
