---
title: "SMTP Headers for Tracking, Tags and Sandbox"
sidebarTitle: "Special Headers"
description: "Reference for AhaSend custom SMTP headers for open and click tracking, tags, retention and sandbox mode, with accepted values and limits."
icon: "heading"
---
...
Header names are case-insensitive. This page uses `AhaSend-` casing for readability.
...
Header names are case-insensitive. See the [SMTP language examples](/smtp) for setting them in your library.
...
<AccordionGroup>
  <Accordion title="Message Metadata & Delivery Log Retention" icon="chart-line">
    ...
    **Values:** 1 day to your [plan's maximum](/retention)<br />
  ...
  <Accordion title="Message Data Retention" icon="envelope">
    ...
    **Values:** Within your [plan's range](/retention); 0 needs approval<br />
...
<AccordionGroup>
  ...
  <Accordion title="Data Management Headers" icon="database">
    - `ahasend-metadata-retention`: Integer within your plan's range - metadata and delivery log retention in days
    - `ahasend-data-retention`: Integer within your plan's range - email content retention in days; 0 needs approval
...
<CardGroup cols={2}>
  <Card title="Header Processing" icon="gear">
    🗸 All AhaSend headers are removed before delivery<br />
    🗸 Recipients never see these special headers<br />
    🗸 Headers only affect AhaSend's processing behavior
  </Card>

  <Card title="Priority & Overrides" icon="arrow-up">
    🗸 Per-email headers override account settings<br />
    🗸 Headers take effect immediately<br />
...
- [Domain setup](/domains), [SMTP credentials](/smtp/credentials), [SMTP settings](/smtp).
...
- [Plans and limits](/facts), [Tracking setup](/tracking/open-tracking), [Suppression durations](/api-reference/webhooks#suppression-duration).
