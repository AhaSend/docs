...
AhaSend retention settings control how long message metadata and content remain available; this guide explains the defaults, plan ranges and per-message overrides.

Available on Free, Pro and Max; the retention ranges depend on your plan, and zero message-data retention needs approval. See [plans and features](/facts#plans-and-features).
...
<AccordionGroup>
  ...
  <Accordion title="Message Data" icon="envelope">
    ...
    **Retention:** Within your plan's range; 0 days needs approval
    ...
    <Warning>
    **Message routing needs message data.** Keep message data retention at 1 day or more on any account that uses [message routing](/integrations/routing).
    </Warning>
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Set Default Periods" icon="calendar">
    **Configure retention periods:**
    ...
    <Warning>
    **Minimum Requirements:** Message metadata requires at least 1 day retention for delivery tracking. Message data can be set to 0 days on Pro and Max after approval from AhaSend. Accounts that use message routing must keep message data for at least 1 day: routes deliver inbound emails from stored message data and stop working at 0 days.
    </Warning>
  </Step>
...
- [API authentication](/api-reference/authentication), [Send-only API keys](/send-api/credentials), [Account settings API](/api-reference/accounts/update-account).
- [SMTP retention headers](/smtp/special-headers), [API message retention](/api-reference/messages/create-message), [S3 archiving](/retention/s3).
