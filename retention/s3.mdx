---
title: "Archive Emails to S3-Compatible Storage"
sidebarTitle: "S3 Archiving"
description: "Archive every AhaSend email (.eml) and delivery log to your own S3, Backblaze B2, DigitalOcean Spaces, Wasabi or MinIO bucket for retention beyond 30 days."
icon: "aws"
---


AhaSend S3 archiving copies email messages and delivery logs to S3-compatible storage; this guide covers setup and long-term retention.
...
<Steps>
  ...
  <Step title="Generate Access Credentials" icon="key">
    ...
    **Security:** Use dedicated credentials with minimal permissions (write only).
    ...
    <Info>
    Grant only the object permissions required by your selected S3-compatible provider.
    </Info>
  </Step>
  ...
  <Step title="Verify Setup" icon="check">
    **Confirm archiving is working:**
    ...
    2. **Wait for asynchronous processing**; this page does not promise an archive completion time
...
<AccordionGroup>
  ...
  <Accordion title="Backblaze B2" icon="database">
    ...
    **Benefits:**
    ...
    - Check the provider's current storage and transfer terms
    - S3-compatible API
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Missing Archives" icon="question">
    **Why messages might not be archived:**
    ...
    - **Processing delays:** Archiving runs asynchronously; files may not appear immediately

    **Verification Steps:**
    1. Check message data retention settings
    2. Verify S3 configuration in account settings
    3. Check the archive bucket and account notifications. If a file is missing, contact support with the message ID; do not send storage credentials
  ...
  <Accordion title="File Access Issues" icon="key">
    ...
    **File Retrieval Examples:**
    ...
    aws s3 cp "s3://your-bucket/john_AT_example.com/<YYYY-MM-DD>/<MESSAGE_ID>/<MESSAGE_ID>.eml" ./

    # Download message with + character in email
    aws s3 cp "s3://your-bucket/user_PLUS_tag_AT_example.com/<YYYY-MM-DD>/<MESSAGE_ID>/<MESSAGE_ID>.eml" ./
...
- [API authentication](/api-reference/authentication), [Send-only API keys](/send-api/credentials), [Retention settings](/retention).
- [Account settings API](/api-reference/accounts/update-account), [Message API](/api-reference/messages/get-message), [Plans and limits](/facts).
