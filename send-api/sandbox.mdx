...
description: "Test AhaSend API v2 sends without delivering mail: use sandbox keys or \"sandbox\": true, simulate bounces, deferrals and failures, and trigger webhooks for free."
icon: "flask"
---
...
<CardGroup cols={2}>
  <Card title="What Happens in Sandbox" icon="check">
    ...
    ✅ No real SMTP delivery attempts are made<br />
    ✅ Relevant webhooks include representative attempt details<br />
...
<AccordionGroup>
  <Accordion title="Webhook Events Still Fire" icon="webhook">
    ...
    - Test delivery confirmations, bounces, deferrals, failures, and suppressions. Sandbox messages cannot generate real opens or clicks; use [test webhook events](/cli/webhook-testing) for those payloads
    - Verify your webhook endpoint receives correct payloads
    - Delivery, deferral, and bounce webhooks include a representative `delivery_attempt` object

    The event flow and payload shape mirror production, but delivery-attempt
    diagnostics are synthetic because no destination mail server was contacted.
    A sandbox classification may override the representative sample and therefore
    may not agree with its SMTP code or response text.
  </Accordion>
