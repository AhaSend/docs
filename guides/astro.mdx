...
- An Astro 5 or later project on Node.js 22.12.0 or a later supported even-numbered release
...
<Note>
  The SDK runs on the Node adapter and on Cloudflare's `workerd` runtime. Edge runtimes need `@ahasend/sdk` 0.2.0 or later; for Cloudflare specifics see the [Cloudflare Workers guide](/guides/cloudflare-workers). Astro's Vercel adapter runs on-demand routes in a serverless function; its optional Edge Function runs middleware only.
</Note>
...
Astro endpoints receive a standard `Request`, so `verifier.parse()` can take `request.headers` and the raw body bytes directly, no adapter needed. `parse()` is asynchronous as of 0.2.0, so await it. Bound the body while reading it: verification rejects bodies over 30 MB, but calling `request.text()` or `request.arrayBuffer()` first would buffer an oversized unauthenticated request before that check runs.
