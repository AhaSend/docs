...
<Note>
  The SDK runs on the Node adapter and on edge adapters alike (Cloudflare, Vercel Edge), so pick whichever suits your host. Edge adapters need `@ahasend/sdk` 0.2.0 or later; for Cloudflare specifics see the [Cloudflare Workers guide](/guides/cloudflare-workers).
</Note>
...
Astro endpoints receive a standard `Request`, so `verifier.parse()` takes `request.headers` and the raw body from `request.text()` directly, no adapter needed. `parse()` is asynchronous as of 0.2.0, so await it. Read the body as text, not JSON, so the verifier sees the exact bytes AhaSend signed.
