...
A [Gin](https://gin-gonic.com) handler wraps a standard `*http.Request` inside `*gin.Context`, so unwrap it: `c.Request` for `verifier.ParseRequest`. The trap is Gin's binding and logging middleware, which must not read the webhook body first.
