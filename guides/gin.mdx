...
sidebarTitle: "Gin"
description: "Send transactional email from a Gin (Go) app with the AhaSend Go SDK: a BasicAuth-protected send route, typed error handling, and verified webhooks."
...
This guide shows how to use AhaSend to send transactional email from Gin in Go, keep credentials on the server, and test a sandbox send.
...
func sendWelcome(c *gin.Context) {
	...
	idempotencyKey := "sandbox-welcome-" + hex.EncodeToString(digest[:])
