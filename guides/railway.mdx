---
title: "How to Send Email from Railway (Node.js)"
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) with the `messages:send:{yourdomain.com}` scope for your sending domain, and your account ID

## What You're Deploying

Install `@ahasend/sdk` and `express` in a Node.js 22 or newer project. This complete server uses a dedicated server-to-server token and sends in sandbox mode. Replace the sender with a verified domain.
...
const token = process.env.WELCOME_ENDPOINT_TOKEN;
if (!token) throw new Error("WELCOME_ENDPOINT_TOKEN is required");
const expected = createHash("sha256").update(token).digest();
const app = express();

app.get("/health", (_req, res) => { res.sendStatus(200); });
app.post("/api/welcome", express.json({ limit: "16kb" }), async (req, res) => {
  const authorization = req.get("authorization") ?? "";
  const supplied = authorization.startsWith("Bearer ") ? authorization.slice(7) : "";
  if (!supplied || !timingSafeEqual(createHash("sha256").update(supplied).digest(), expected)) {
    res.sendStatus(401);
    return;
  }
  const { email, eventId } = req.body ?? {};
  if (typeof email !== "string" || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ||
      typeof eventId !== "string" || !/^[A-Za-z0-9._:-]{1,200}$/.test(eventId)) {
    res.sendStatus(400);
    return;
  }
  try {
    const result = await ahasend.messages.send({
      ...
      sandbox: true,
    }, { idempotencyKey: `sandbox-welcome-${eventId}` });
    const rejected = result.data.some((entry) => entry.status === "error");
    res.status(rejected ? 422 : 202).json({ accepted: !rejected });
  } catch (error) {
    if (error instanceof AhaSendAPIError) {
      console.error("Send failed", { status: error.status, requestId: error.requestId });
    }
    res.sendStatus(502);
  }
});
if (process.env.AHASEND_WEBHOOK_SECRET) {
  app.post("/webhooks/ahasend", expressWebhookHandler(
    new WebhookVerifier(process.env.AHASEND_WEBHOOK_SECRET),
    async (_event, _req, res) => { res.statusCode = 204; res.end(); },
    { maxBodyBytes: 30_000_000 },
  )); // Verification only; add a durable handoff before business side effects.
}
...
Run TypeScript with your project's runner (for example `npx tsx server.ts`). Call `/api/welcome` with your server token and JSON containing `email` and a stable `eventId`. Add a per-caller send limit before production use. An AhaSend 202 contains one result per recipient; this route returns 422 if any recipient is rejected.
