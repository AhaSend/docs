...
sidebarTitle: "Hono"
description: "Send transactional email from a Hono app on Node.js or Bun with the AhaSend TypeScript SDK: protected send route, idempotent retries, and signed webhooks."
...
```bash bun
bun add @ahasend/sdk hono @hono/node-server
```
  ...
  async (c) => {
    ...
    try {
      const result = await ahasend.messages.send(
        ...
        { idempotencyKey: `sandbox-welcome-${input.eventId}` },
      );
...
`nextRouteHandler` is the SDK's web-standard Request/Response adapter, so it works with Hono's `c.req.raw`.
...
Let a failed commit reject: the adapter rethrows a handler error, Hono answers 500, and AhaSend retries. Never catch it into a 200. Every non-2xx answer counts as a failed delivery — [delivered with 6 attempts over about 16 minutes, with the webhook disabled after more than 100 consecutive failed attempts](/api-reference/webhooks/retry-policy) — so the opaque 400 the adapter returns for a bad signature quietly spends that budget on every delivery until the secret is fixed.
...
- **Scheduling**: `first_attempt` must be RFC3339, in the future, and within 7 days. set `schedule: { first_attempt: new Date(Date.now() + 60_000).toISOString() }` to defer delivery.
...
- **Attachments**: pass `attachments: [{ data, content_type, file_name, base64: true }]`. For binary files such as PDFs, base64-encode the bytes yourself and set `base64: true`.
