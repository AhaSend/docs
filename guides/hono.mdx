...
sidebarTitle: "Hono"
description: "Send transactional email from a Hono app on Node.js, Bun, or Cloudflare Workers with the AhaSend SDK, plus signed webhook verification."
keywords: ["hono send email", "hono transactional email", "hono email api", "ahasend hono", "hono webhook email"]
---

Hono's portability now extends to the SDK: from 0.2.0 the same handler runs on Node.js, Bun, and Workers unchanged. The samples below use `@hono/node-server`, and the webhook route calls the generic verifier directly.
...
<Note>
  Deploying Hono to Cloudflare Workers, Vercel Edge, or another edge platform? That works from SDK 0.2.0 onward, and Cloudflare workerd does not need `nodejs_compat`. On edge platforms read the credentials from the platform's own bindings (`env` in Workers) and pass them to the constructor. See the [Cloudflare Workers guide](/guides/cloudflare-workers) for the Workers specifics.
</Note>
...
app.post("/webhooks/ahasend", async (c) => {
  let event;
  try {
    event = await verifier.parse(c.req.raw.headers, await c.req.text());
  } catch (err) {
...
<AccordionGroup>
  <Accordion title="Constructor throws about an unsupported runtime">
    On an edge target (Cloudflare Workers, Vercel Edge), you are on an SDK version older than 0.2.0, which is where edge support landed: upgrade to 0.2.0 or later. In a browser the constructor still refuses to run by design, because the API key must never reach client code. Supported server runtimes are Node.js 22, 24 and 26, Deno, Bun, Cloudflare workerd, and Vercel Edge.
  </Accordion>
