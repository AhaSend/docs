...
This example pins `@supabase/server` to 1.4.1. Its [withSupabase helper](https://supabase.github.io/server/functions/index.withSupabase.html) passes the authenticated Supabase client and user claims to the handler.
  ...
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    try {
      const result = await ahasend.messages.send(
        ...
        { idempotencyKey: `sandbox-welcome-${environment}-${userId}` },
      );
