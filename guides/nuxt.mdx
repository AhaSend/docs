...
Signature verification needs the **exact request bytes**. The SDK's web-standard adapter reads and bounds the unconsumed request stream, verifies the signature and timestamp, parses the event, and returns opaque client errors for invalid deliveries:
...
let route: ReturnType<typeof fromWebHandler> | undefined;
...
<AccordionGroup>
  <Accordion title="Webhook verification always returns 400">
    Something consumed the request body before `nextRouteHandler` could read the exact bytes the HMAC was computed over, or the configured webhook secret does not match this dashboard endpoint. Keep body parsing middleware off this route so `fromWebHandler` can pass the unconsumed request stream to the SDK adapter.
  </Accordion>
