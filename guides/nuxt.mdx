...
This guide shows how to send transactional email from Nuxt with AhaSend, keep credentials on the server, and test a sandbox send.
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) with the `messages:send:{your-domain}` scope (`messages:send:all` for sending from many domains), and your account ID
...
Create the client on first use and reuse it across requests:
...
Signature verification needs the **exact request bytes**. The Node-server example below reads `event.node.req` directly and counts bytes before buffering them. Do not run `readBody()` or other body-reading middleware first: parsing and re-serializing JSON changes the signed bytes. Answer every failed check with an empty body so nothing about the failure reaches the sender:
...
const MAX_WEBHOOK_BYTES = 30_000_000;

function readLimitedWebhookBody(event: H3Event): Promise<Buffer | null> {
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      if (total > MAX_WEBHOOK_BYTES) {
        cleanup();
        request.pause();
        // Send 413 before closing the unread connection.
...
export default defineEventHandler(async (event) => {
  ...
  const rawBody = await readLimitedWebhookBody(event);
  if (rawBody === null || rawBody.byteLength === 0) {
    setResponseStatus(event, rawBody === null ? 413 : 400);
    return null;
  }
...
<AccordionGroup>
  ...
  <Accordion title="401 AhaSendAuthenticationError">
    The API key never reached the AhaSend SDK client on the server. Check that `nuxt.config.ts` declares `ahasendApiKey` in `runtimeConfig` and that the env var is named exactly `NUXT_AHASEND_API_KEY`, since Nuxt only maps variables whose names match the config key. Restart the dev server after editing `.env`.
  </Accordion>
