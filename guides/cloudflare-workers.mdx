...
<Warning>
  The 0.2.0 README and release gates list workerd as maintained, but the published 0.2.0 `SECURITY.md` limits production security support to Node.js 22+ and says edge runtimes are outside that boundary. If that support boundary is a requirement, run the SDK on a supported Node.js backend until the policy is reconciled.
</Warning>
...
```bash yarn
yarn add --exact @ahasend/sdk@0.2.0
```
