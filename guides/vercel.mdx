...
AhaSend scopes an idempotency key to your account and matches it against the
exact request body — not against the `sandbox` flag. That is why `welcomeKey`
puts `sandbox` or `live` in the key itself: without it, a Preview sandbox send
and a Production live send sharing one signup ID would reuse a single key with
two different bodies, which the API rejects as a 422 mismatch.
