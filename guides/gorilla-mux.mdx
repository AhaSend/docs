...
[gorilla/mux](https://github.com/gorilla/mux), maintained again since 2023, routes plain `net/http` handlers, so `verifier.ParseRequest` takes the `*http.Request` you already receive. The care goes into `r.Use(...)`, which must leave the webhook body untouched.
