---
title: "How to Send Transactional Email with Gorilla Mux (Go)"
...
Send your first transactional email from a [gorilla/mux](https://github.com/gorilla/mux) app in under five minutes using the official AhaSend Go SDK, then wire up webhooks to react to deliveries, bounces, and opens in real time. gorilla/mux is actively maintained again since 2023 under new stewardship, and its handlers are plain `net/http`, so everything here is standard library plus the SDK.
...
Create the client once at startup and share it across handlers — it maintains its own rate-limit, retry, and idempotency state. Pass the router to `http.ListenAndServe` to serve it:
...
func main() {
	...
	r := mux.NewRouter()
	r.HandleFunc("/api/welcome", sendWelcome).Methods("POST")
	r.HandleFunc("/webhooks/ahasend", handleAhaSendWebhook).Methods("POST")
...
gorilla/mux hands you the raw `*http.Request`, so the SDK's `ParseRequest` works directly — it verifies the HMAC signature, guards against replays, and returns a typed event.
...
<AccordionGroup>
  ...
  <Accordion title="405 Method Not Allowed on my routes">
    `.Methods("POST")` restricts the route to POST only — a GET to the same path returns 405. Make sure your test request uses POST, and note that method names are case-sensitive uppercase (`"POST"`, not `"post"`).
  </Accordion>
