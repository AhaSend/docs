...
`verifier.parse()` accepts the Fetch `Headers` object from `request.headers` directly, and it is asynchronous as of 0.2.0, so await it. Keep the webhook route free of `body` schemas and of plugins/hooks that read `body`: once Elysia parses the body, the request stream is consumed and `request.text()` returns an empty string, which fails verification.
