---
title: "How to Send Email with ElysiaJS (Bun)"
sidebarTitle: "ElysiaJS"
description: "Send transactional email from an ElysiaJS app on Bun with the AhaSend SDK: a typed t.Object send route, auth in onRequest, and verified signed webhooks."
...
This guide shows how to use AhaSend to send transactional email from ElysiaJS in Bun, keep credentials on the server, and test a sandbox send.
...
function isAuthorized(header: string | null) {
  ...
  return provided !== "" && timingSafeEqual(createHash("sha256").update(provided).digest(), expectedTokenHash);
}
  ...
  .post(
    ...
    },
    {
      body: t.Object({
        ...
        eventId: t.String({ pattern: "^[A-Za-z0-9._:-]{1,200}$" }),
      }),
    },
  );
```
