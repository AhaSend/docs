...
`verifier.parse()` accepts the Fetch `Headers` object from `request.headers` directly, and it is asynchronous as of 0.2.0, so await it. Keep `parse: "none"`, and keep the webhook route free of plugins or hooks that consume `request.body` before the handler.
