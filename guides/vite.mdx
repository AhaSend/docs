...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) permitted to send from that domain, and your account ID
...
// The adapter reads the raw stream; do not put a global body parser before it.
app.post("/webhooks/ahasend", expressWebhookHandler(
  new WebhookVerifier(process.env.AHASEND_WEBHOOK_SECRET),
  async (_event, _req, res) => { res.status(204).end(); },
  { maxBodyBytes: 30_000_000 },
)); // Verification only; add durable deduplication before side effects.

// Register this after every route.
...
and proxies `/api` into the loopback backend, so the authentication middleware and the per-user rate
limit you add protect a live API key from becoming an open mail relay for
...
<AccordionGroup>
  ...
  <Accordion title="import.meta.env.VITE_AHASEND_API_KEY is visible">
    ...
    [dashboard](https://dash.ahasend.com/account/-/transactional/credentials). Keep the
