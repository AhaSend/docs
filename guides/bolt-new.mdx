...
Bolt and Supabase server functions use the edge-function path below. AhaSend SDK 0.2.0 added Deno and edge runtime support; this guide pins that currently published version. On Node hosts the SDK needs Node.js 22 or later.

<Warning>
  The SDK 0.2.0 README, changelog, and release gates list Deno and edge runtimes as maintained, but its published security policy still limits security support to Node.js 22 or later and lists Deno and edge runtimes as outside that boundary. If your deployment requires an explicitly security-supported SDK runtime, use a separately deployed Node.js 22+ backend until that policy is reconciled.
</Warning>

## Send with the SDK

Install an exact SDK version, which handles retries, idempotency, and typed errors. The SDK is still pre-1.0, so pin it and review the changelog before upgrading:

<CodeGroup>

```bash npm
npm install --save-exact @ahasend/sdk@0.2.0
```

```bash pnpm
pnpm add --save-exact @ahasend/sdk@0.2.0
```

```bash bun
bun add --exact @ahasend/sdk@0.2.0
```

</CodeGroup>

In a Bolt or Supabase server function there is nothing to install in the app package: import the pinned package with `import { AhaSendClient } from "npm:@ahasend/sdk@0.2.0"`.
