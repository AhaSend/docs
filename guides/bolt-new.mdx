...
description: "Add transactional email to a Bolt.new app with the AhaSend SDK from a Bolt or Supabase server function or Node backend, with the API key kept server-side."
...
This guide shows how to send transactional email from a Bolt.new server function with AhaSend, keep credentials on the server, and test a sandbox send.
