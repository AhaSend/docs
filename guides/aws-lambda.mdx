---
title: "How to Send Email from AWS Lambda (Node.js)"
sidebarTitle: "AWS Lambda"
description: "Send transactional email from AWS Lambda with the AhaSend Node.js SDK: a protected send handler, esbuild bundling, idempotent retries, and signed webhooks."
...
This guide shows how to use AhaSend to send transactional email from AWS Lambda in Node.js, keep credentials on the server, and test a sandbox send.
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) with a domain scope such as `messages:send:{yourdomain.com}` matching the domain in `from.email` (the curly braces are part of the [scope](/api-reference/scopes) string), and your account ID
...
For Python, use the [AWS Lambda Python guide](/guides/aws-lambda-python).
