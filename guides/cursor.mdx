...
- For work that can be retried outside one SDK call, persist the exact send
  ...
  account and matched against the request body, so a key reused with a
  different body is rejected with 422, not replayed. Namespace keys by
  environment, since flipping `sandbox` changes the body. Stored message outcomes
...
The rule is now included in model context for every Agent and Inline Edit request. Rules guide model behavior rather than enforcing it, so confirm that **ahasend-email** is listed under Customize → Rules and review the resulting code against it.
