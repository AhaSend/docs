...
```bash npm
npm install @ahasend/sdk @nestjs/config class-transformer class-validator
```

```bash pnpm
pnpm add @ahasend/sdk @nestjs/config class-transformer class-validator
```

```bash yarn
yarn add @ahasend/sdk @nestjs/config class-transformer class-validator
```

```bash bun
bun add @ahasend/sdk @nestjs/config class-transformer class-validator
```
...
<AccordionGroup>
  <Accordion title="req.rawBody is undefined">
    Nest only captures the raw body when you pass `{ rawBody: true }` to `NestFactory.create`. Without it, this controller rejects the request. If you switch to the Fastify adapter, follow Nest's Fastify raw-body setup and use Fastify's request type instead of Express's `Request` type.
  </Accordion>
