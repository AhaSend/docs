...
Then verify and dispatch in a controller. `verifier.parse()` is asynchronous as of 0.2.0, so the handler awaits it:
...
export class WebhookController {
  ...
  async handle(@Req() req: RawBodyRequest<Request>) {
    let event;
    try {
      event = await verifier.parse(req.headers, req.rawBody!);
    } catch (err) {
