---
title: "How to Send Transactional Email with NestJS"
sidebarTitle: "NestJS"
...
Send your first transactional email from a [NestJS](https://nestjs.com) (v10 or v11) app using the official AhaSend Node.js SDK, wrapped in an injectable service, then verify webhooks with the SDK's generic verifier.
...
<Warning>
The API key grants full send access to your account. Keep it server-side: never expose it to any client-facing code.
</Warning>
...
See the [API reference](https://ahasend.com/docs/api-reference) for every endpoint the SDK exposes (`client.domains`, `client.suppressions`, `client.statistics`, and more). Nest runs on Express or Fastify under the hood, so the standalone [Express](/guides/express) and [Fastify](/guides/fastify) guides show the SDK's built-in webhook adapters for those platforms.
