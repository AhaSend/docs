...
Do not use a `NEXT_PUBLIC_` prefix. Add the variables to the environments where the application runs, mark `AHASEND_API_KEY` as **Sensitive** for Preview and Production, and redeploy. Vercel makes environment variables available during builds and function execution, and changes apply only to new deployments.

Next.js only includes variables prefixed with `NEXT_PUBLIC_` in the browser bundle. Project members with sufficient Vercel access can still view ordinary Vercel environment variables, which is why the API key should use the Sensitive option.
...
Create the client once at module scope. This keeps retry, telemetry, idempotency, and local pacing configuration consistent across routes, and lets sends in one process share the SDK's process-local pacing queue.
...
<AccordionGroup>
  ...
  <Accordion title="An environment variable is undefined">
    The variable may be missing from the deployment's selected environment or may have been added after its last deployment. Correct the environment assignment and redeploy.
  </Accordion>
