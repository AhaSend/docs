---
title: "How to Send Email from a v0 App"
sidebarTitle: "v0"
description: "Send transactional email from a v0-generated Next.js app with AhaSend: a server-side Route Handler, Vercel environment variables, and sandbox tests."
...
```bash npm
npm install @ahasend/sdk server-only
```

```bash pnpm
pnpm add @ahasend/sdk server-only
```

```bash yarn
yarn add @ahasend/sdk server-only
```

```bash bun
bun add @ahasend/sdk server-only
```
