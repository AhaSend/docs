---
title: "How to Send Email from Google Cloud Functions (Go)"
sidebarTitle: "Cloud Run Functions"
description: "Send transactional email from Google Cloud Run functions (formerly Cloud Functions) in Go with the AhaSend Go SDK, Secret Manager, and signed webhooks."
...
This guide shows how to use AhaSend to send transactional email from Google Cloud Run functions in Go, keep credentials on the server, and test a sandbox send.
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) with the domain-scoped `messages:send:{your-domain}` scope matching the domain in `From.Email` (use `messages:send:all` only if the key must cover several domains), and your account ID
...
func sendWelcome(w http.ResponseWriter, r *http.Request) {
	...
	response, _, err := client.MessagesAPI.CreateMessage(
		...
		accountID,
		message,
		api.WithIdempotencyKey("sandbox-welcome-"+eventID.String()),
	)
	if err != nil {
...
`go126` is an example runtime ID. Choose an available runtime at least as new as the `go` directive in your `go.mod`; a newer local toolchain may write a directive the selected builder cannot compile. `gcloud functions runtimes list --region=us-central1` shows what your project can deploy today.
...
<AccordionGroup>
  ...
  <Accordion title="401 from the AhaSend API">
    The API key is missing, malformed, or revoked. Verify the secret binding (`--set-secrets AHASEND_API_KEY=...`) is present on the function and the key exists in your [dashboard](https://dash.ahasend.com/account/-/transactional/credentials).
  </Accordion>
...
Other language guides: [Cloud Run functions with Node.js](/guides/google-cloud-functions-nodejs) and [Cloud Run functions with Python](/guides/google-cloud-functions-python).
