---
title: "How to Send Email from Google Cloud Functions in Go"
sidebarTitle: "Google Cloud Functions"
description: "Send transactional email from Google Cloud Functions in Go with the Functions Framework and the AhaSend email API, plus verified delivery webhooks."
keywords: ["google cloud functions go send email", "golang transactional email cloud functions", "cloud run functions go email", "ahasend go sdk", "cloud functions email api"]
---

Send transactional email from Google Cloud Functions in Go — now [Cloud Run functions](https://cloud.google.com/functions) (2nd gen) — using the AhaSend Go SDK to send messages and receive delivery webhooks. The [Functions Framework for Go](https://github.com/GoogleCloudPlatform/functions-framework-go) hands your code a standard `http.ResponseWriter` and `*http.Request`, so the SDK — including `verifier.ParseRequest` — works with no adaptation.
...
## Write the Google Cloud Functions code
...
<Tip>
Return 200 promptly and do slow processing asynchronously (Cloud Tasks or Pub/Sub work well here) — AhaSend retries webhooks that don't get a timely 2xx.
</Tip>
