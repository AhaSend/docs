...
The SDK's release gates run its packed artifact on the latest Deno 2.x. Deno Deploy can run npm packages without a Node build step, but its managed runtime may lag that tested version, and the SDK security policy currently limits production security support to Node.js 22 or later. Validate the pinned SDK on Deno Deploy before production and treat Deno compatibility separately from security-fix support.

## Prerequisites

- Deno 2 locally, and a [Deno Deploy](https://console.deno.com) organization and application
...
The SDK is pre-1.0, so resolve and save an exact version rather than allowing an inline specifier to drift:

```bash
deno add --save-exact npm:@ahasend/sdk
```

This writes the exact npm version to `deno.json`; commit both `deno.json` and `deno.lock`, and review the SDK changelog before upgrading. Import the saved bare specifier:
...
For local development, create `.env` yourself and ensure it is ignored by git before adding values. Run with only the permissions this example needs:
...
The same `Deno.serve` handler API runs locally and on Deno Deploy, although the runtime version and permission model differ as noted above:
...
`webhookDeliveries.enqueueOnce` is application-owned: in one database transaction it must insert the unique verified `webhook-id` and durable work, returning `false` for an already-committed ID. Process that work separately with idempotent side effects. Do not merely insert the ID before doing work, and do not log recipient addresses, event/error objects, signatures, bodies, secrets, or idempotency keys.
