---
title: "How to Send Email with Deno and Deno Deploy"
sidebarTitle: "Deno Deploy"
description: "Send transactional email from Deno and Deno Deploy with the AhaSend SDK via npm specifiers: a Deno.serve send route, Deploy secrets, and verified webhooks."
...
import QuickSend from "/snippets/quick-send-deno.mdx";
import SendGuideRules from "/snippets/send-guide-rules.mdx";

This guide shows how to send transactional email from Deno and Deno Deploy with AhaSend, keep credentials on the server, and test a sandbox send.
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) scoped to `messages:send:{your-domain}`, and your account ID
...
import { handleAhaSendWebhook } from "./webhook.ts";
// Application-owned: implement signup lookup and authorization before using real mail.
...
Deno.serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/welcome") {
    ...
    try {
      const result = await ahasend.messages.send(
        ...
        { idempotencyKey: `sandbox-welcome-${user.signupId}` },
      );
  ...
  if (req.method === "POST" && url.pathname === "/webhooks/ahasend") {
    return handleAhaSendWebhook(req);
  }
...
The send example uses `sandbox: true` to validate the request without delivering anything. Sandbox and live sends share one idempotency namespace per account, and `sandbox` is part of the request body, so flipping it while reusing a key raises `AhaSendIdempotencyMismatchError` instead of sending.
...
The example uses the SDK's 30,000,000-byte ceiling, including for inbound `message.routing` events with attachments. Keep the proxy limit in step and cap concurrent requests. A 413 counts as a failed attempt; [more than 100 consecutive failed attempts disable the webhook](/api-reference/webhooks/retry-policy).

```ts webhook.ts
import { AhaSendWebhookVerificationError, WebhookVerifier, isKnownWebhookEvent } from "@ahasend/sdk/webhooks";
// Application-owned: provide atomic durable storage as described below.
import { webhookDeliveries } from "./lib/webhook-store.ts";

const verifier = new WebhookVerifier(Deno.env.get("AHASEND_WEBHOOK_SECRET")!);
const MAX_WEBHOOK_BODY_BYTES = 30_000_000;
...
export async function handleAhaSendWebhook(req: Request): Promise<Response> {
  let event;
  try {
  ...
  if (!isKnownWebhookEvent(event)) return new Response(null, { status: 204 });
