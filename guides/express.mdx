...
app.post(
  "/api/welcome",
  (req, res, next) => {
    if (req.get("authorization") !== `Bearer ${welcomeEndpointToken}`) {
...
The SDK retries transient failures automatically, but a retry after a 5xx can still send twice. When a duplicate would be expensive, pass a stable key from your business operation as the second argument: `ahasend.messages.send(message, { idempotencyKey: "welcome-user-123" })`.
