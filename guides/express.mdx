---
title: "How to Send Transactional Email with Express"
...
Send your first transactional email from an [Express](https://expressjs.com) app in under five minutes using the official AhaSend Node.js SDK, then wire up webhooks to react to deliveries, bounces, and opens in real time.
...
See the [API reference](https://ahasend.com/docs/api-reference) for every endpoint the SDK exposes (`client.domains`, `client.suppressions`, `client.statistics`, and more). On a different Node framework? The same SDK powers the [Fastify](/guides/fastify) and [NestJS](/guides/nestjs) guides.
