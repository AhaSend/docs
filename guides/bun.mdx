...
sidebarTitle: "Bun"
description: "Send transactional email from a Bun.serve route with the AhaSend TypeScript SDK, with no shims or dotenv, and verify signed AhaSend webhooks in Bun."
...
import QuickSend from "/snippets/quick-send-bun.mdx";
import SendGuideRules from "/snippets/send-guide-rules.mdx";

This guide shows how to send transactional email from Bun with AhaSend, keep credentials on the server, and test a sandbox send.
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method === "POST" && url.pathname === "/api/welcome") {
      ...
      if (typeof eventId !== "string" || !/^[A-Za-z0-9._:-]{1,200}$/.test(eventId)) {
      ...
      try {
        ...
        return Response.json({ statuses: result.data.map((r) => r.status) }, { status: rejected.length ? 422 : 202 }); // ["queued"]
      } catch (err) {
...
Narrow the catch to `AhaSendWebhookVerificationError` and rethrow anything else. Every non-2xx answer counts as a failed delivery: [6 attempts over about 16 minutes, with a webhook disabled after more than 100 consecutive failed attempts](/api-reference/webhooks/retry-policy). A bug of your own should therefore surface as a 5xx rather than as a rejection that looks like a bad signature, which quietly spends that budget.
