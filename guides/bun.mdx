...
There's no Bun-specific adapter, and you don't need one: `verifier.parse()` accepts a Fetch `Headers` object and a raw body string directly, which is exactly what a Bun `Request` gives you. `parse()` is asynchronous as of 0.2.0, so await it. Read the body with `req.text()`, not `req.json()`, so the verifier sees the exact bytes AhaSend signed.
...
<AccordionGroup>
  ...
  <Accordion title="SDK works locally but you plan to deploy to an edge platform">
    Edge platforms are supported from `@ahasend/sdk` 0.2.0 onwards, Cloudflare workerd included, with no `nodejs_compat` flag. See the [Cloudflare Workers guide](/guides/cloudflare-workers) for the details there.
  </Accordion>
</AccordionGroup>
