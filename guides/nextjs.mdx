---
title: "How to Send Email with Next.js (App Router)"
sidebarTitle: "Next.js"
description: "Send transactional email from Next.js App Router route handlers and Server Actions with the AhaSend SDK, keep API keys server-side, and verify webhooks."
...
Use Next.js 15.1 or later for the stable [`after()` API](https://nextjs.org/blog/next-15-1).
...
- An [API key](https://dash.ahasend.com/account/-/transactional/credentials) with the `messages:send:{yourdomain.com}` scope matching the domain in `from.email` (or `messages:send:all` to cover every domain), and your account ID
...
```bash npm
npm install @ahasend/sdk zod server-only
```

```bash pnpm
pnpm add @ahasend/sdk zod server-only
```

```bash yarn
yarn add @ahasend/sdk zod server-only
```

```bash bun
bun add @ahasend/sdk zod server-only
```
...
export async function sendWelcomeEmail(input: WelcomeInput) {
  const idempotencyKey = `sandbox-welcome-${createHash("sha256")
...
<AccordionGroup>
  <Accordion title="401 AhaSendAuthenticationError">
    The API key is missing, malformed, or revoked. Verify `AHASEND_API_KEY` is set in `.env.local` (restart `next dev` after editing it) and that the key exists in your [dashboard](https://dash.ahasend.com/account/-/transactional/credentials). In production, set the variable in your hosting provider's environment settings.
  </Accordion>
