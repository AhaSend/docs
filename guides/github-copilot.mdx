...
description: "Delegate an AhaSend email integration to the GitHub Copilot coding agent: write the issue, review the PR, and test safely in sandbox mode."
...
This guide shows how to send transactional email from an app built with GitHub Copilot with AhaSend, keep credentials on the server, and test a sandbox send.
...
Add this shared block to .github/copilot-instructions.md:
...
Create a domain-scoped credential in **Sandbox** mode under **Credentials → Add → API Key v2** for the Agents secret. It simulates sending even when generated code omits `sandbox: true`; see [sandbox credentials](/send-api/sandbox#how-do-i-enable-sandbox-mode). Keep production credentials out of agent test environments. Keep the agent firewall enabled and explicitly allow `api.ahasend.com` if the request is blocked; do not disable the firewall just to make the test pass.
