...
Create a domain-scoped credential in **Sandbox** mode under **Credentials → Add → API Key v2** for the Agents secret. It simulates sending even when generated code omits `sandbox: true`; see [sandbox credentials](/send-api/sandbox#creating-sandbox-credentials). Keep production credentials out of agent test environments. Keep the agent firewall enabled and explicitly allow `api.ahasend.com` if the request is blocked; do not disable the firewall just to make the test pass.
