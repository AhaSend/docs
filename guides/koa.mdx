---
title: "How to Send Email with Koa (Node.js)"
sidebarTitle: "Koa"
...
This guide shows how to use AhaSend to send transactional email from Koa in Node.js, keep credentials on the server, and test a sandbox send.
...
import { createAhaSendWebhookRouter, memoryWebhookStore } from "./lib/webhook.js";
import { ahasend, deliveryMode, sandboxMode, welcomeToken, requireEnv } from "./lib/ahasend.js";
...
const webhookRouter = createAhaSendWebhookRouter(requireEnv("AHASEND_WEBHOOK_SECRET"), memoryWebhookStore);
...
const WEBHOOK_BODY_LIMIT_BYTES = 30_000_000;
...
// Local demo only. Replace with a durable store before adding side effects.
const events = new Map<string, AnyWebhookEvent>();
export const memoryWebhookStore: WebhookStore = {
  async enqueueOnce(id, event) {
