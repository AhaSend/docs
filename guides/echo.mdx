...
sidebarTitle: "Echo"
description: "Send transactional email from an Echo v5 (Go) app with the AhaSend Go SDK: KeyAuth and rate-limit middleware, sandbox sends, and verified webhooks."
...
This guide shows how to use AhaSend to send transactional email from Echo in Go, keep credentials on the server, and test a sandbox send.
...
```bash
go mod init example.com/ahasend-echo
...
func main() {
	...
	e.POST("/webhooks/ahasend", handleAhaSendWebhook, middleware.BodyLimit(30_000_000))
