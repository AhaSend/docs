...
func sendWelcome(c *echo.Context) error {
	expected := []byte("Bearer " + sendToken)
	provided := []byte(c.Request().Header.Get("Authorization"))
	if sendToken == "" || subtle.ConstantTimeCompare(provided, expected) != 1 {
	...
	if in.Email == "" || !eventIDPattern.MatchString(in.EventID) {
		return echo.NewHTTPError(http.StatusBadRequest, "email and valid event_id are required")
	}
	...
	if len(response.Data) != 1 || response.Data[0].Status == "error" {
		log.Printf("AhaSend rejected a recipient count=%d", len(response.Data))
		return echo.NewHTTPError(http.StatusBadGateway, "recipient rejected")
	}
...
The example defaults to sandbox mode, which validates without delivery. Change `sandbox` to `false` only when you intend to send real mail. Its stable business-event key protects later calls for the same logical request, while the mode prefix prevents a recent sandbox outcome from being replayed for a live payload. Keep an `event_id` bound to the same request data.
