...
Signature verification needs the **raw request body**, which Encore's typed endpoints parse away. Use `api.raw` instead: its handler receives a Node `IncomingMessage`, so you collect the body chunks into a Buffer and hand them to the verifier along with the headers. `verifier.parse()` is asynchronous as of 0.2.0, so await it:
...
export const ahasendWebhook = api.raw(
  ...
  async (req, resp) => {
    ...
    try {
      const event = await verifier.parse(req.headers, rawBody);
