...
- Node.js and the [Encore CLI](https://encore.dev/docs/ts/install)
...
The lazy module-level singleton means the client is constructed once per process. The SDK still generates a new automatic idempotency key for each logical call and reuses that key only for that call's internal retries.
