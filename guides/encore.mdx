---
title: "How to Send Transactional Email with Encore.ts"
...
Sending transactional email with Encore.ts is fully supported by the AhaSend SDK, since [Encore.ts](https://encore.dev) runs your backend on Node.js. This guide wires the SDK into Encore's typed API endpoints, stores credentials with Encore's built-in secrets manager, and verifies webhooks in a raw endpoint.
...
<Tip>
The lazy module-level singleton means the client is constructed once per process: after the first request it's reused, keeping the SDK's retry and idempotency state intact.
</Tip>
