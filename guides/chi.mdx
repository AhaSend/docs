...
sidebarTitle: "chi"
description: "Send transactional email from a Go chi router with the AhaSend Go SDK: a token-protected send handler, idempotent retries, and HMAC-verified webhooks."
...
```bash
go mod init example.com/ahasend-chi
...
- **Rate limits**: keep each client at or below [100 requests per second with a 200 burst](/api-reference/rate-limits), and coordinate the total across all processes.
