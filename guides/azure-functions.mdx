...
On Azure, a Go function is a **custom handler**: a small HTTP server the Functions host forwards requests to. Ordinary `net/http` code works, including the SDK's `verifier.ParseRequest`.
...
Return 200 promptly and do slow processing asynchronously, because AhaSend retries webhooks that don't get a timely 2xx.
