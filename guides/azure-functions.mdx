---
title: "How to Send Email from Azure Functions (Go)"
sidebarTitle: "Azure Functions"
description: "Send transactional email from Azure Functions in Go with a custom handler and the AhaSend Go SDK, plus HMAC-verified delivery webhooks and a Linux deploy."
...
This guide shows how to use AhaSend to send transactional email from Azure Functions in Go, keep credentials on the server, and test a sandbox send.

This guide runs Go as an Azure Functions **custom handler**: a small HTTP server the Functions host forwards requests to. Ordinary `net/http` code works, including the SDK's `verifier.ParseRequest`. As of September 2026, Microsoft documents first-class Go support as a public preview and recommends it for new Go function apps. Check the [Azure custom-handler documentation](https://learn.microsoft.com/en-us/azure/azure-functions/functions-custom-handlers) and its linked Go preview requirements before choosing a hosting plan.
...
func handleAhaSendWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, webhooks.ErrUnknownEventType):
			...
			// it: AhaSend disables a webhook after more than 100 consecutive failed attempts.
...
Other language guides: [Azure Functions with Node.js](/guides/azure-functions-nodejs) and [Azure Functions with Python](/guides/azure-functions-python).
