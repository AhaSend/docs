---
title: "Install the AhaSend CLI (macOS, Linux, Windows)"
description: "Install the AhaSend CLI on macOS, Linux, or Windows from a prebuilt binary, with go install, or from source, then enable shell completion and verify it works."
icon: download
...
- **Go Version**: 1.26.1 or later (for building from source)
...
Linux and Windows ARM64 require `go install` or a source build.
...
Build the public source to contribute or target another supported platform. Contacts require a later CLI release; the supplied local development build includes them, but public v0.2.0 and its current public source do not.
...
```bash
# Using Make
sudo cp bin/ahasend /usr/local/bin/ahasend
...
```bash
go install github.com/AhaSend/ahasend-cli@latest
```

This installs a binary named `ahasend-cli` into `GOBIN`, or `$(go env GOPATH)/bin` when `GOBIN` is unset. Rename it to `ahasend` to use the commands in these docs. With the default Go bin directory:

```bash
mv "$(go env GOPATH)/bin/ahasend-cli" "$(go env GOPATH)/bin/ahasend"
```
...
Download the binary and `checksums.txt` from the same [release](https://github.com/AhaSend/ahasend-cli/releases). On Linux, verify the binary before installing it:

```bash
sha256sum --ignore-missing --check checksums.txt
```

On macOS, use `shasum -a 256 ahasend-darwin-arm64` (or the Intel filename) and compare its result to the matching line in `checksums.txt`. The releases page lists the current version.
...
# Add to PATH (User scope; no admin rights needed)
...
ahasend completion bash | sudo tee /etc/bash_completion.d/ahasend > /dev/null
...
**Linux:**
```bash
# Install completion for your user
mkdir -p ~/.zsh/completions
ahasend completion zsh > ~/.zsh/completions/_ahasend
...
go install github.com/AhaSend/ahasend-cli@latest
mv "$(go env GOPATH)/bin/ahasend-cli" "$(go env GOPATH)/bin/ahasend"
"$(go env GOPATH)/bin/ahasend" --help
```
...
- [CLI authentication](/cli/authentication), [REST sending guide](/send-api/send-email), [SMTP settings](/smtp).
- [Node.js SDK](/guides/nodejs-sdk), [Go SDK](/guides/go-sdk), [CLI configuration and output](/cli/configuration).
- [API authentication](/api-reference/authentication), [Permission scopes](/api-reference/scopes).
