---
title: "Test Webhooks Locally with the CLI"
description: "Receive AhaSend webhook events on localhost without a public URL: stream them with ahasend webhooks listen, forward them and trigger tests."
icon: bug
...
An existing webhook must be enabled to emit events. For a disabled test webhook, start its listener, then run `ahasend webhooks update YOUR_RESOURCE_ID --enable` in another terminal. Disable it again before stopping the listener. A listener without `--webhook-id` creates an enabled temporary webhook.
...
**Signature Verification Failed**: The listener prints its signing secret when it starts. Store that secret in your local handler's configuration; use the full string as the raw UTF-8 HMAC key. Do not base64-decode it. See [signature verification](/api-reference/webhooks/security). The command automatically generates a webhook secret using the Standard Webhooks headers and format, with the full secret as raw UTF-8 bytes
...
- [Event reference](/api-reference/webhooks), [Webhook CLI](/cli/commands/webhooks), [Signature verification](/api-reference/webhooks/security).
