---
title: "API Rate Limits and 429 Handling"
sidebarTitle: "Rate Limits"
description: "AhaSend API v2 allows 100 requests/second per account (burst 200); statistics endpoints allow 1/second. Learn what a 429 response looks like and how to retry."
icon: "gauge-high"
---


AhaSend API rate limits apply per account; this reference lists request limits, explains HTTP 429, and shows how to retry.
...
**Affected Endpoints:** all other v2 endpoints, including `/v2/ping`, account members, sub-accounts and sub-account usage.
...
### Statistics, Stream and Trigger Endpoints

Statistics, webhook and route stream, and test-trigger endpoints have a separate limit of 1 request per second:
...
<Info>
Requests share the account limit. Spread requests over time; a new second does not reset a fixed request counter. Do not depend on `Retry-After` or `X-RateLimit-*` headers for rate-limit responses.
</Info>
...
Hourly sending limits start low for new accounts and grow with account age, plan and sending reputation; support can raise them. These are separate from API request rates and the Free plan's 1,000 included emails per month. See [plans and limits](/facts).
