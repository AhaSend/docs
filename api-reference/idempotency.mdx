...
4. **Automatic Cleanup**: Idempotency keys expire after 24 hours
...
Claiming an idempotency key happens in a short database transaction guarded by a PostgreSQL advisory lock; the lock is never held while your request is being processed. While a request is in flight, concurrent requests with the same key receive `409 Conflict` with a `Retry-After` header instead of waiting.
