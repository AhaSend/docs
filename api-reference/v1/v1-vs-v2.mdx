---
title: "API v1 vs v2: Differences and Migration"
sidebarTitle: "v1 vs v2"
description: "Compare AhaSend API v1 and v2 side by side (auth headers, request fields, responses, features) and follow the step-by-step migration from /v1/email/send to v2."
icon: "code-compare"
---
  ...
  <Tab title="API v2">
    ...
    ```json
    {
      ...
      "schedule": {
        ...
        "expires": "<RFC3339 expiry within the next 8 days>"
      }
    }
    ```
  </Tab>
...
For the v2 URL, copy the account UUID from **Account Settings → API Keys** or use `ahasend auth status`. See [account ID and authentication](/api-reference/authentication). Scheduled first attempts must be in the future and within 7 days; expiry must be in the future and within 8 days.
...
- [API errors](/api-reference/errors), [API rate limits](/api-reference/rate-limits), [Retention rules](/retention).
