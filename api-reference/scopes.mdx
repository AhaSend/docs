...
sidebarTitle: "Scopes"
description: "Every AhaSend API key scope, from messages:send:all to domain-specific scopes for webhooks, routes and SMTP, and how each request is authorized."
icon: "lock"
---


AhaSend API key scopes control which operations and domains a key can access; this reference lists the scope names and their rules.

Available on Free, Pro and Max. See [plans and features](/facts#plans-and-features).


In a domain scope such as `messages:send:{example.com}`, keep the braces and replace only `example.com` with your domain. The braces are part of the scope string.
...
| `domains:delete:all` | Delete any domain in the account |
| `domains:delete:{domain}` | Delete a specific domain |

<Warning>
  Domain deletion accepts `domains:delete:all` or the matching `domains:delete:{domain}` scope.
</Warning>
...
These scopes are granted to **parent**-account API keys to manage sub-accounts and the API keys owned by those sub-accounts. See the [Sub-Accounts API](/api-reference/sub-accounts/overview) for the full workflow.
...
<Note>
  Sub-account API keys are managed only with parent credentials that hold the relevant `sub-account-api-keys:*` scope. A sub-account's own credentials cannot create or manage its nested API keys.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Separate API Keys" icon="key">
    ...
    A separate reporting or management key can use:
...
- [API authentication](/api-reference/authentication), [Send-only API keys](/send-api/credentials), [REST sending guide](/send-api/send-email).
