---
title: "Reputation Shield: Block Disposable Email"
sidebarTitle: "Reputation Shield"
description: "AhaSend Reputation Shield blocks mail to 200,000+ toxic and disposable domains and to typo domains like gmial.com, protecting your reputation."
icon: "shield-halved"
---


AhaSend Reputation Shield blocks sends to high-risk recipient addresses; this guide explains the checks and how to configure them.
...
- **API:** Check both the HTTP status and each message's `status` in the response, as shown below.
...
```plaintext
553 <human-readable rejection reason>
```
...
<Tabs>
  ...
  <Tab title="API Integration">
    Use [Create Message](/api-reference/messages/create-message) with Node.js 18 or later. Replace the API key, account ID and sender address with your own values. This example uses sandbox mode so no email reaches the recipient.
    ...
    async function sendEmail(recipient) {
      ...
      for (const message of result.data) {
        if (message.status === 'error') {
          throw new Error(`${message.recipient.email}: ${message.error}`);
        }
      }
    ...
    sendEmail('user@example.com')
      .then(messages => console.log('Message results:', messages))
      .catch(error => console.error('Email sending failed:', error.message));
    ```

    A queued message returns HTTP `202` with this response shape:
    ...
    ```json
    {
      ...
      "data": [
        {
          "object": "message",
          "id": "<message-id@yourdomain.com>",
          "recipient": { "email": "user@example.com", "name": "" },
          "status": "queued",
          "error": null
        }
      ]
    ...
    For a request with several recipients, HTTP `202` can include rejected recipients with `status: "error"`, `id: null` and an `error` message. Check every entry in `data` before treating the whole request as successful.

    If no recipient can be queued, the API currently returns HTTP `500` with this body:

    ```json
    { "message": "failed to queue messages" }
    ```

    That response does not identify the rejection reason. Error text can change, so do not rely on matching phrases such as "toxic or tempmail" to decide whether Reputation Shield blocked an address.
  </Tab>
</Tabs>
...
- [Account quickstart](/quickstart), [Domain setup](/domains), [Send through API](/send-api/send-email).
- [SMTP special headers](/smtp/special-headers), [Suppression durations](/api-reference/webhooks#suppression-duration), [Plans and limits](/facts).
- [Tracking setup](/tracking/open-tracking), [Sandbox behavior](/send-api/sandbox).
