---
title: "IP Allow Lists for API Keys"
sidebarTitle: "IP Allow Lists"
description: "Lock AhaSend API keys to specific IPv4/IPv6 addresses or CIDR ranges (up to 100 entries) so a leaked key can't be used outside your infrastructure."
icon: "network-wired"
---


AhaSend API key IP allow lists limit where a key can be used; this guide shows how to add allowed addresses and avoid locking yourself out.

Available on Free, Pro and Max. See [plans and features](/facts#plans-and-features).

<Info>
**Scope of this feature:** IP allow lists apply to Full API keys only. SMTP credentials and sending credentials are not affected.
</Info>
...
```json
{
  "message": "Request rejected by the API key IP allow list"
}
```
...
- [Scoped credentials](/security/scoped-credentials), [API key CLI](/cli/commands/apikeys), [Create key API](/api-reference/api-keys/create-api-key).
